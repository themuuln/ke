...
fn main() -> anyhow::Result<()> {
    ...
    match cmd {
        ...
        Some("install") => { print_install(); Ok(()) }
        Some("status") | Some("st") => { cli_status(); Ok(()) }
        Some("init") => { cli_init(&args[2..]); Ok(()) }

        // CLI commands (delegate to the same backend)
        Some("set") | Some("s") => { cli_set(&args[2..]); Ok(()) }
        Some("get") | Some("g") => { cli_get(&args[2..]); Ok(()) }
        Some("list") | Some("ls") | Some("l") => { cli_list(&args[2..]); Ok(()) }
        Some("load") => { cli_load(&args[2..]); Ok(()) }
        Some("cat") | Some("c") => { cli_cat(&args[2..]); Ok(()) }
        Some("cp") => { cli_cp(&args[2..]); Ok(()) }
        Some("delete") | Some("del") | Some("d") => { cli_delete(&args[2..]); Ok(()) }
        Some("pull") | Some("pl") => { cli_pull(&args[2..]); Ok(()) }
        Some("push") | Some("ph") => { cli_push(&args[2..]); Ok(()) }
        Some("run") | Some("r") => { cli_run(&args[2..]); Ok(()) }
        Some("rm") => { cli_rm_project(&args[2..]); Ok(()) }
...
fn print_install() {
    println!("ke is already installed at: {}", std::env::current_exe().unwrap_or_default().display());
    println!();
...
fn cli_get(args: &[String]) {
    ...
    let val = keychain::Keychain::get(&args[0], &args[1])
        .unwrap_or_else(|| {
            eprintln!("Error: {}:{} not found", args[0], args[1]);
            std::process::exit(1);
        });
    println!("{}", val);
}
...
fn cli_load(args: &[String]) {
    ...
    let config = Config::load().unwrap();
    let keys = config.list_keys(project).unwrap_or_default();
    println!("# {} — from macOS Keychain", project);
    for k in &keys {
...
fn cli_cp(args: &[String]) {
    ...
    eprintln!("Copied {}:{} to clipboard!", args[0], args[1]);
}
...
fn cli_delete(args: &[String]) {
    ...
    match keychain::Keychain::delete(&args[0], &args[1]) {
        Ok(()) => {
            let config = Config::load().unwrap();
            let _ = config.remove_key(&args[0], &args[1]);
            eprintln!("Deleted {}:{}", args[0], args[1]);
        }
        Err(e) => {
            eprintln!("Error: {}", e);
            std::process::exit(1);
...
fn cli_pull(args: &[String]) {
    ...
    let config = Config::load().unwrap();
    let keys = config.list_keys(project).unwrap_or_default();
    let out = std::env::current_dir().unwrap().join(".env.local");
    let mut content = format!("# {} — pulled from Keychain on\n", project);
    ...
    eprintln!("Wrote {} ({} secrets)", out.display(), count);
}
...
fn cli_push(args: &[String]) {
    ...
    for line in content.lines() {
        ...
        if let Some((key, val)) = line.split_once('=') {
            ...
            if !key.is_empty() && !val.is_empty() {
                if keychain::Keychain::set(project, key, val).is_ok() {
                    let _ = config.add_key(project, key);
                    count += 1;
                }
            }
    ...
    eprintln!("Pushed {} secrets from {} into Keychain for '{}'", count, file, project);
}
...
fn cli_run(args: &[String]) {
    ...
    let cmd_start = if cmd_args.first() == Some(&"--") { 1 } else { 0 };
...
fn cli_status() {
    ...
    let statuses = config.status().unwrap_or_default();
